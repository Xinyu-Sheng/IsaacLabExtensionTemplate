...
- Throughout the repository, the name `your_fancy_extension_name` only serves as an example and we provide a script to rename all the references to it automatically:

```bash
# Enter the repository
cd IsaacLabExtensionTemplate
# Rename all occurrences of your_fancy_extension_name (in files/directories) to your_fancy_extension_name
...
```bash
python -m pip install -e exts/your_fancy_extension_name
```
...
```json
{
    "python.analysis.extraPaths": [
        "<path-to-ext-repo>/exts/your_fancy_extension_name"
    ]
}
```
